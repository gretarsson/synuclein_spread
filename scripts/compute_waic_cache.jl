...
if mode == :shuffle
    sim_paths = filter(f -> occursin(r"largeu0_DIFFGA_shuffle_\d+$", splitext(basename(f))[1]), all_files)
elseif mode == :seed
    sim_paths = filter(f -> occursin(r"largeu0_DIFFGA_seed_\d+$", splitext(basename(f))[1]), all_files)
else
...
serialize("results/waic_cache/largeu0_DIFFGA_$(String(mode))_waic_all.jls", waic_vals)
