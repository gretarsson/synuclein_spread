...
timepoints = vec(readdlm(file_time, ','))[1:endpoint]
plt = StatsPlots.plot()
for i in 1:N
...
@model function fitlv(data, prob; alg=alg, u0_prior_avg=u0_prior_avg, u0_prior_std=u0_prior_std, timepoints=timepoints, proper_idxs=proper_idxs, seed=seed)
    ...
    σ ~ InverseGamma(2, 3)
    ρ ~ truncated(Normal(1.0, 0.5); lower=0.)
    α ~ truncated(Normal(1.0, 0.5); lower=0.)  # scalar
    β ~ truncated(Normal(1.0, 0.5); lower=0.)  # scalar
    ...
    #u0 = [0 for _ in 1:N]
    #u0[seed] ~ truncated(Normal(0.,0.1), lower=0.)  # scalar (seed) 
...
for (key,value) in vars
    ...
    save("figures/aggregation_inference/aggregation_data_less/chains/chain_$(key).png",chain_plot_i)
    i += 1
end
