...
const OUTPUT_DIR = get(ENV, "COPATHOLOGY_OUTDIR", "figures/copathology_updated_growth/amyloid_beta")
...
function setup_defaults()
    ...
    default(size=FIG_SIZE, guidefontsize=LABEL_SIZE, tickfontsize=TICK_SIZE, titlefontsize=TITLE_SIZE,
        legendfontsize=12, left_margin=4mm, right_margin=4mm, top_margin=4mm, bottom_margin=4mm)
end
...
function app_parameter_df()
    ...
    beta_flags = region_update_flags(inf_app, "beta")
    gamma_flags = region_update_flags(inf_app, "gamma")

    df = DataFrame(
        region = labels,
        beta_app = beta_app,
        gamma_app = gamma_app,
        beta_updated = [get(beta_flags, r, 0) == 1 for r in labels],
        gamma_updated = [get(gamma_flags, r, 0) == 1 for r in labels],
    )
...
function scatter_panel(df::DataFrame, xcol::Symbol, ycol::Symbol; xlabel::AbstractString, ylabel::AbstractString, title::AbstractString)
    ...
    x = df[mask, xcol]
    y = df[mask, ycol]
    plt = scatter(x, y; xlabel=xlabel, ylabel=ylabel, title=title, markercolor=:black,
        markeralpha=0.55, markersize=6, markerstrokewidth=0.0, label=false)
    if length(x) >= 3
...
function association_table(df::DataFrame)
    ...
    rows = DataFrame(abeta_measure=String[], parameter=String[], n=Int[], pearson_r=Float64[],
        pearson_p=Float64[], spearman_rho=Float64[], spearman_p=Float64[])
    for (ameas, xcol, pname, ycol, updated_col) in combos
        mask = .!isnan.(df[!, xcol]) .& .!isnan.(df[!, ycol]) .& df[!, updated_col] .& (df.beta_app .> 0)
        x = df[mask, xcol]
        y = df[mask, ycol]
        if length(x) < 3
            continue
...
function main()
    ...
    CSV.write(joinpath(CSV_DIR, "regional_abeta_syn_app_parameter_table_updated_growth.csv"), df)

    stats_df = association_table(df)
    CSV.write(joinpath(CSV_DIR, "abeta_syn_app_association_stats_updated_growth.csv"), stats_df)

    dfβ = df[df.beta_updated .& (df.beta_app .> 0), :]
    dfγ = df[df.gamma_updated .& (df.beta_app .> 0), :]

    ab40_panel = plot(
        scatter_panel(dfβ, :ab40_log10p1, :beta_app; xlabel="log10(1 + Ab40 burden)", ylabel="APP syn rise parameter β", title="Ab40 vs APP syn β (updated + β>0)"),
        scatter_panel(dfγ, :ab40_log10p1, :gamma_app; xlabel="log10(1 + Ab40 burden)", ylabel="APP syn fall parameter γ", title="Ab40 vs APP syn γ (updated + β>0)");
        layout=(1, 2), size=(1200, 600))
    savefig(ab40_panel, joinpath(OUTPUT_DIR, "ab40_syn_app_associations_updated_growth.pdf"))

    ab42_panel = plot(
        scatter_panel(dfβ, :ab42_log10p1, :beta_app; xlabel="log10(1 + Ab42 burden)", ylabel="APP syn rise parameter β", title="Ab42 vs APP syn β (updated + β>0)"),
        scatter_panel(dfγ, :ab42_log10p1, :gamma_app; xlabel="log10(1 + Ab42 burden)", ylabel="APP syn fall parameter γ", title="Ab42 vs APP syn γ (updated + β>0)");
        layout=(1, 2), size=(1200, 600))
    savefig(ab42_panel, joinpath(OUTPUT_DIR, "ab42_syn_app_associations_updated_growth.pdf"))

    combined_panel = plot(
        scatter_panel(dfβ, :abeta_mean_z, :beta_app; xlabel="Mean z-score of log Aβ burden", ylabel="APP syn rise parameter β", title="Mean Aβ vs APP syn β (updated + β>0)"),
        scatter_panel(dfγ, :abeta_mean_z, :gamma_app; xlabel="Mean z-score of log Aβ burden", ylabel="APP syn fall parameter γ", title="Mean Aβ vs APP syn γ (updated + β>0)");
        layout=(1, 2), size=(1200, 600))
    savefig(combined_panel, joinpath(OUTPUT_DIR, "combined_abeta_syn_app_associations_updated_growth.pdf"))
