...
waic_dict = Dict{String, Float64}()
for sp in sim_paths
    base = splitext(basename(sp))[1]   # e.g. DIFFGA_seed_12
    waic_file = joinpath("results/waic_cache", base * "_waic.jls")

    if isfile(waic_file)
        w = deserialize(waic_file)
        if isfinite(w)
            waic_dict[basename(sp)] = w
        end
    end
end
waic_dict
