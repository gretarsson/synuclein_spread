...
for i in eachindex(thresholds)
    ...
    #priors_agg["ϵ"] = truncated(Normal(0,1),lower=0)
    #priors_agg["d"] = truncated(Normal(0,1),lower=0)
    priors_agg["σ"] = InverseGamma(2,3)
    priors_agg["seed"] = truncated(Normal(0,1),lower=0)
    #priors_agg["c"] = truncated(Normal(0,1),lower=0)
    factors_diff = [1/100, 1.]
    factors_diff4 = [1/100, 1., 1.]
    factors_agg = [1/100, 1., 10., [1/10 for _ in 1:N]...]
    #factors_agg = [1/100, 1., [10. for _ in 1:N]..., [1. for _ in 1:N]..., [1. for _ in 1:N]..., 1., 1.]
    #factors_agg = [1/100, 1., 10., [1. for _ in 1:N]..., [1. for _ in 1:N]..., 1/10.]
    ...
    serialize("simulations/total_death_local2_N=$(N).jls", inference)
end
