...
const OUTPUT_DIR = get(ENV, "COPATHOLOGY_OUTDIR", "figures/copathology_updated")
const TOP_N = 20
...
function scatter_figure(df::DataFrame, family::String)
    ...
    text!(
        ax,
        minimum(x) + 0.06 * (maximum(x) - minimum(x) + eps()),
...
function lollipop_figure(df::DataFrame, family::String; top_n::Int=TOP_N)
    ...
    ax = Axis(
        fig[1, 1],
        title = "$(uppercase(family)) regions with largest APP - MAPT shifts",
        xlabel = "Posterior mean shift (APP - MAPT)",
        ylabel = "",
        titlesize = 28,
        xlabelsize = 24,
        xticklabelsize = 18,
...
function main()
    ...
    for path in (
        beta_scatter_paths.pdf_path,
        beta_scatter_paths.png_path,
        gamma_scatter_paths.pdf_path,
        gamma_scatter_paths.png_path,
        beta_shift_paths.pdf_path,
        beta_shift_paths.png_path,
        gamma_shift_paths.pdf_path,
        gamma_shift_paths.png_path,
    )
        println(path)
    end
end
